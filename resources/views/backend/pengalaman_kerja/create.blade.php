                ...
                <div class="col-lg-12">
                    <section class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form method="POST"
                                action="{{ isset($pengalaman) ? route('pengalaman_kerja.update', $pengalaman->id) : route('pengalaman_kerja.store') }}"
                                id="pengalaman_kerja_form">
                                @csrf
                                @if(isset($pengalaman))
                                    @method('PUT') <!-- Jika edit, gunakan PUT -->
                                @endif

                                <div class="mb-3">
                                    <label for="nama" class="form-label">Nama Perusahaan</label>
                                    <input type="text" class="form-control" id="nama" name="nama"
                                        value="{{ old('nama', $pengalaman->nama ?? '') }}" minlength="5" required>
                                </div>

                                <div class="mb-3">
                                    <label for="jabatan" class="form-label">Jabatan</label>
                                    <input type="text" class="form-control" id="jabatan" name="jabatan"
                                        value="{{ old('jabatan', $pengalaman->jabatan ?? '') }}" minlength="2" required>
                                </div>

                                <div class="mb-3">
                                    <label for="tahun_masuk" class="form-label">Tahun Masuk</label>
                                    <input type="text" class="form-control datepicker" id="tahun_masuk" name="tahun_masuk"
                                        value="{{ old('tahun_masuk', $pengalaman->tahun_masuk ?? '') }}" required>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control datepicker" id="tahun_keluar" name="tahun_keluar"
                                        value="{{ old('tahun_keluar', $pengalaman->tahun_keluar ?? '') }}" required>
                                </div>
